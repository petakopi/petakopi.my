class CoffeeShopsController < ApplicationController
  ...
  def show
    ...
    last_7_days_check_ins = CheckIn.includes(:user).where(created_at: 7.days.ago.., coffee_shop: @coffee_shop)
    @check_ins = {
      count: last_7_days_check_ins.count,
      users: last_7_days_check_ins.order(created_at: :desc).limit(4).map(&:user)
    }
