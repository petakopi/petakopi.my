class Admin::CoffeeShopOwnersController < AdminController
  ...
  def index
    @coffee_shop_owners = CoffeeShopOwner.includes(:coffee_shop, :user).order(created_at: :desc)
  end
  ...
  def create
    ...
    if @coffee_shop_owner.save
      redirect_to admin_coffee_shop_owners_url,
        notice: "Coffee shop owner for #{@coffee_shop_owner.coffee_shop.slug} was successfully created."
    else
      set_data
